...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initI18n();
    initializeLandingRedesign();
});

function initializeLandingRedesign() {
    initializePerfectForTicker();
    initializeBenefitsReveal();
    initializeBenefitsCarousel();
}

function initializePerfectForTicker() {
    const el = document.getElementById('perfectForWord');
    if (!el) return;

    const words = ['Airbnbs', 'Property Managers', 'Blogs', 'Venues', 'Museums', 'Theatres', 'Arenas', 'Transportation Hubs'];
    let currentIndex = 0;
...
function initializeBenefitsReveal() {
    const cards = document.querySelectorAll('.benefits-track .feature-card');
    ...
    const grid = document.querySelector('.benefits-track');
    if (grid) {
